...
export default function MlOrders() {
    ...
    return (
        <div className="space-y-6 p-4 md:p-0">
            <Head title="Diamond Orders" />
            <div>
                <h1 className="text-2xl font-bold text-gray-900">Diamond Orders</h1>
                ...
                <table className="min-w-full text-sm">
                    <thead className="bg-gray-50 text-gray-600">
                        <tr>
                            ...
                            <th className="text-left px-4 py-3">ML Account</th>
                            ...
                            <tr key={o.id} className="border-t">
                                ...
                                <td className="px-4 py-3">
                                    <div className={`flex items-center gap-2 ${copiedRow === o.id ? "text-green-500 font-medium" : ""}`}>
                                        {copiedRow === o.id ? "Copied!" : `ID: ${o.ml_account_id || "-"}`}
                                        <button
                                            className="cursor-pointer text-gray-500"
                                            onClick={() => handleCopy(o.ml_account_id+`(${o.ml_server_id})`, o.id )}
                                        >
                                            <IoCopyOutline />
                                        </button>
                                    </div>
                                    <div className="text-xs text-gray-500">Server: {o.ml_server_id || "-"}</div>
                                </td>
                    ...
                    <div key={o.id} className="bg-white rounded-xl border border-gray-200 p-4 shadow-sm">
                        ...
                        <div className="grid grid-cols-2 gap-y-3 gap-x-2 text-sm">
                            ...
                            <div>
                                ...
                                <p className="text-xs text-gray-500 italic">Diamond: {o.diamonds ?? "-"}</p>
                            </div>
                            ...
                            <div className="col-span-2 bg-gray-50 p-2 rounded-lg border border-dashed border-gray-200">
                                <div className="w-full flex items-center justify-between">
                                    <p className="text-[10px] text-gray-400 uppercase font-bold">ML Account</p>
                                    <button
                                        className="cursor-pointer text-gray-500"
                                        onClick={() => handleCopy(o.ml_account_id, o.id)}
                                    >
                                        <IoCopyOutline />
                                    </button>
                                </div>
                                <div className="flex justify-between items-center">
                                    <span className="font-mono font-bold text-indigo-600">
                                        {copiedRow === o.id ? "Copied!" : o.ml_account_id || "-"}
                                    </span>
                                    <span className="text-xs text-gray-600">Server: {o.ml_server_id || "-"}</span>
                                </div>
                            </div>

...
export default function ServiceOrders() {
    ...
    const handleCopy = async (text, rowId) => {
        if (!text) return;
        await navigator.clipboard.writeText(text);
        setCopiedRow(rowId);
        setTimeout(() => setCopiedRow(null), 1300);
    };
                        ...
                        {orders.map((o) => (
                            <tr key={o.id} className="border-t">
                                ...
                                <td className="px-4 py-3">
                                    ...
                                    <div className="text-xs  font-bold text-indigo-600">Value: {o.value ?? "-"}</div>
                                </td>
                                <td className="px-4 py-3">
                                    <div className={`flex items-center gap-2 ${copiedRow === o.id ? "text-green-500 font-medium" : ""}`}>
                                        {copiedRow === o.id ? "Copied!" : o.target_telegram_id || "-"}
                                        <button
                                            className="cursor-pointer text-gray-400 hover:text-gray-600"
                                            onClick={() => handleCopy(o.target_telegram_id, o.id)}
                                        >
                                            <IoCopyOutline size={14} />
                                        </button>
                                    </div>
                                </td>
                                <td className="px-4 py-3 font-medium">{formatAmount(o.sell_price, o.sell_currency)}</td>
                                <td className="px-4 py-3">{Number(o.profit_base ?? 0).toLocaleString("fr-FR")}</td>
                                <td className="px-4 py-3">
                                    <span className={`px-2 py-1 rounded-full text-[11px] font-semibold ${statusClass[o.status] || "bg-gray-100 text-gray-700"}`}>
                                        {o.status}
                                    </span>
                                </td>
                                <td className="px-4 py-3">
                                    <div className="flex gap-2">
                                        <button
                                            ...
                                            className="px-3 py-1.5 text-xs rounded-lg bg-emerald-600 text-white hover:bg-emerald-700 disabled:bg-black/10 disabled:text-gray-400 disabled:cursor-not-allowed"
                                        >
                                            Qabul qilish
                                        </button>
                                        <button
                                            ...
                                            className="px-3 py-1.5 text-xs rounded-lg bg-rose-600 text-white hover:bg-rose-700 disabled:bg-black/10 disabled:text-gray-400 disabled:cursor-not-allowed"
                                        >
                                            Bekor
                                        </button>
                                    </div>
                ...
                {orders.map((o) => (
                    <div key={o.id} className="bg-white rounded-xl border border-gray-200 p-4 shadow-sm">
                        <div className="flex justify-between items-center mb-3">
                            ...
                            <span className={`px-2 py-1 rounded-full text-[10px] font-semibold uppercase tracking-wider ${statusClass[o.status] || "bg-gray-100 text-gray-700"}`}>
                                {o.status}
                            </span>
                        </div>
                        ...
                        <div className="grid grid-cols-2 gap-y-3 gap-x-2 text-sm">
                            ...
                            <div>
                                ...
                                <p className="font-medium">{o.service_title || "-"}</p>
                                <p className="text-[11px] text-gray-500 italic">{o.service_type || "-"}</p>
                                <p className="text-[11px] font-bold text-indigo-600">Value: {o.value ?? "-"}</p>
                            </div>

                            <div>
                                <p className="text-xs text-gray-400 uppercase font-medium">Narx</p>
                                <p className="font-bold text-emerald-600">{formatAmount(o.sell_price, o.sell_currency)}</p>
                            </div>

                            <div className="col-span-2 bg-gray-50 p-2 rounded-lg border border-dashed border-gray-200">
                                <div className="w-full flex items-center justify-between">
                                    <p className="text-[10px] text-gray-400 uppercase font-bold">Target Username</p>
                                    <button
                            ...
                            <div className="col-span-2 flex justify-between items-center text-xs text-gray-400 pt-1">
                                <span className={o.status === "canceled" ? "line-through" : ""}>
                                    Foyda: {Number(o.profit_base ?? 0).toLocaleString("fr-FR")} UZS
                                </span>

...
import { CheckCircle, Clock, Copy, Receipt, Upload, X } from "lucide-react";
...
const METHODS = [
    { key: "binance", label: "Binance Pay", accent: "yellow", disabled: true },
    { key: "click", label: "Click", accent: "blue", disabled: true },
...
export default function ResellerBalance() {
    ...
    const [method, setMethod] = useState("chek");
    const ac =
        ACCENT[METHODS.find((m) => m.key === method)?.accent ?? "violet"];
    ...
    useEffect(() => {
        axios
            .get("/binance/rate")
            .then((r) => setUsdtRate(r.data.rate ?? 0))
            .catch(() => {});
        axios
    ...
    const handleClick = async () => {
        ...
        try {
            ...
            const r = await axios.post("/payment/create", {
                order_type: "topup",
                amount: amt,
    ...
    const bnbUsdt =
        usdtRate > 0 && bnbAmt
            ? (parseFloat(bnbAmt) / usdtRate).toFixed(2)
            : null;

    return (
        <>
            <Head title="Balans to'ldirish" />
            <div className="container mx-auto px-4 pt-5 pb-28 lg:pb-10 space-y-4">
                    ...
                    {METHODS.map((m) => {
                        ...
                        return (
                            <button
                                ...
                                className={`relative py-2.5 rounded-xl border text-xs font-bold transition-all ${
                                    m.disabled
                                        ? "border-amber-400/40 bg-amber-400/5 text-slate-600 cursor-not-allowed opacity-70"
                                        : active
                                          ? `${a.tab} bg-white/3`
                            ...
                            >
                                {m.disabled && (
                                    <span className="absolute -top-2 right-1.5 px-1.5 py-0.5 rounded-full bg-amber-400 text-[8px] font-bold text-amber-950 tracking-wide shadow-sm">
                                        {t('balance.coming_soon')}
                                    </span>
                                )}
                                {m.key === "binance" && (
                                    <span
                                        className={`flex items-center justify-center gap-1 ${active ? "text-yellow-300" : ""}`}
                                    >
                                        <BinanceLogo /> Binance
                                    </span>
                                )}
                                ...
                                {m.key === "chek" && (
                                    <span
                                        className={`flex items-center justify-center gap-1 ${active ? "text-violet-300" : ""}`}
                                    >
                ...
                {/* {method === "binance" && (
                    <div className="rounded-2xl border border-white/6 bg-white/2 p-5 space-y-4">
                        ...
                        <div className="space-y-1.5">
                            <label className="text-[10px] text-slate-600 font-bold uppercase tracking-widest block">
                                {t("reseller.amount")}
                            </label>
                            <div className="relative">
                                ...
                                />
                                <span className="absolute right-4 top-1/2 -translate-y-1/2 text-xs text-slate-600 font-bold font-mono">
                                    UZS
                                </span>
                            </div>
                            {bnbUsdt && (
                                <p className="text-xs font-bold text-yellow-400 px-1">
                                    ≈ {bnbUsdt} USDT
                                </p>
                            )}
                        </div>
                        ...
                        <div className="grid grid-cols-3 gap-2">
                            {QUICK.map((a) => (
                                <button
                                    key={a}
                                    onClick={() => setBnbAmt(String(a))}
                                    className={`py-2 rounded-lg text-[11px] font-bold border transition-all ${
                                        bnbAmt === String(a)
                                            ? ac.quick
                                            : "border-white/6 text-slate-600 hover:text-slate-400 hover:bg-white/3"
                                    }`}
                                >
                                    {fmt(a)}
                                </button>
                        ...
                        {usdtRate === 0 && (
                            <p className="text-xs text-amber-500 bg-amber-500/8 border border-amber-500/15 rounded-xl p-3">
                                USDT kursi belgilanmagan. Admin bilan
                                bog'laning.
                            </p>
                        )}
                        ...
                        >
                            {bnbLoading ? (
                                ...
                                <>
                                    ...
                                    {t("reseller.payWith")} Binance Pay
                                </>
                            )}
                        </button>
                    </div>
                ...
                {method === "click" && (
                    <div className="rounded-2xl border border-amber-400/20 bg-amber-400/5 p-6 flex flex-col items-center justify-center text-center gap-2">
                        <img
                            src={ClickLogo}
                            alt="Click"
                        ...
                        <p className="text-xs text-slate-500">
                            Click orqali to'lov hozircha ishlab chiqilmoqda.
                        </p>
                    </div>
                )}
                                            ...
                                            >
                                                ...
                                                <div>
                                                    ...
                                                    <div className="flex items-center justify-between mt-1.5">
                                                        <p className="text-white/50 text-[10px] font-medium uppercase">
                                                            {card.card_holder ??
                                                                "VALLER"}
                                                        </p>
                                                        <button
                        ...
                        <div className="rounded-2xl border border-white/6 bg-white/2 p-4 space-y-4">
                            ...
                            <div className="space-y-3">
                                {[
                                    ...
                                    >
                                        <span className="size-5 rounded-full bg-violet-500/15 border border-violet-500/20 text-[10px] font-black text-violet-400 flex items-center justify-center shrink-0 mt-0.5">
                                            {i + 1}
                                        </span>
                        ...
                        <div className="rounded-2xl border border-white/6 bg-white/2 p-4 space-y-4">
                            {!preview ? (
                                ...
                                <div className="relative rounded-xl border border-white/8 bg-white/3 p-2 flex justify-center">
                                    ...
                                    <button
                                        onClick={removeFile}
                                        className="absolute top-2 right-2 size-6 bg-rose-500 hover:bg-rose-400 text-white rounded-lg flex items-center justify-center transition-all"
                                    >
                                    ...
                                    {history.map((r) => {
                                        ...
                                        return (
                                            ...
                                            >
                                                ...
                                                <span
                                                    className={`text-[10px] font-black uppercase tracking-wider px-2 py-0.5 rounded-lg border ${s.cls}`}
                                                >
                                                    {s.label}
                                                </span>
                                            </div>

        ...
        <div className="min-h-screen bg-[#0a0a0f] text-white select-none">
            ...
            <aside className="hidden lg:flex fixed left-0 top-0 h-full w-60 flex-col bg-[#0f0f1a] border-r border-white/5 z-40">
                ...
                <nav className="flex-1 px-3 space-y-0.5 mt-2">
                    {[
                        { key: "orders",  icon: LayoutDashboard, label: "Dashboard" },
                        { key: "prices",  icon: Tag,             label: "Narxlarim" },
                    ].map(n => (
                        <button
                            key={n.key}
                    ...
                    <Link
                        href="/reseller/shop"
                        className="w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-semibold text-slate-500 hover:text-slate-300 hover:bg-white/3 transition-all"
                    >
            ...
            <main className="lg:ml-60 min-h-screen flex flex-col">
                ...
                {/* Top bar */}
                <header className="sticky top-0 z-30 bg-[#0a0a0f]/80 backdrop-blur-xl border-b border-white/5 px-4 lg:px-8 h-14 flex items-center justify-between">
                    <h1 className="text-sm font-bold text-white hidden lg:block">
                        {tab === "orders" ? "Dashboard" : "Narxlarim"}
                    </h1>
                    ...
                    {tab === "orders" && (
                        <div className="space-y-5 max-w-3xl">
                            ...
                            <div className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-violet-600/20 via-violet-800/10 to-transparent border border-violet-500/15 p-5">
                                ...
                                <div className="relative">
                                    <div className="flex items-start justify-between">
                                        <div>
                                            <p className="text-xs text-slate-500 font-medium uppercase tracking-widest">Joriy balans</p>
                                            <p className="text-3xl font-black text-white mt-1 tracking-tight">
                                                {fmt(balance)}
                                                <span className="text-lg text-slate-500 font-semibold ml-1.5">UZS</span>
                                            </p>
                                        </div>
                                    ...
                                    <div className="flex gap-2 mt-4">
                                        ...
                                        >
                                            <Plus className="size-3.5" /> Balans to'ldirish
                                        </Link>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div className="grid grid-cols-3 gap-2.5">
                                {[
                                    { label: "Jami",        value: stats.total_orders,              suffix: "",    color: "text-slate-300", icon: BarChart3 },
                                    { label: "Bajarilgan",  value: stats.completed,                 suffix: "",    color: "text-emerald-400", icon: TrendingUp },
                                    { label: "Kutilmoqda",  value: stats.pending,                   suffix: "",    color: "text-amber-400", icon: Zap },
                                    { label: "Bugun",       value: fmt(stats.today_spent),          suffix: " UZS", color: "text-blue-400",  icon: TrendingUp },
                                    { label: "Bu oy",       value: fmt(stats.month_spent),          suffix: " UZS", color: "text-violet-400", icon: BarChart3 },
                                    { label: "Tejaldi",     value: fmt(stats.total_saved),          suffix: " UZS", color: "text-emerald-400", icon: TrendingDown },
                                ].map(s => (
                                    <div key={s.label} className="bg-white/3 border border-white/5 rounded-xl p-3 hover:bg-white/5 transition-colors">
                                        ...
                                        <p className={`text-sm font-black mt-1 leading-tight ${s.color}`}>
                                            {s.value}{s.suffix && <span className="text-[10px] font-semibold text-slate-600 ml-0.5">{s.suffix}</span>}
                                        </p>
                                    </div>
                                ))}
                            </div>
                            ...
                            <div>
                                <div className="flex items-center justify-between mb-3">
                                    <h2 className="text-sm font-bold text-slate-400 uppercase tracking-widest">Buyurtmalar</h2>
                                    <span className="text-[10px] text-slate-600">So'nggi 20 ta</span>
                                </div>
                                ...
                                {orders.length === 0 ? (
                                    <div className="flex flex-col items-center justify-center py-16 text-center">
                                        ...
                                        <p className="text-slate-500 text-sm font-medium">Hali buyurtma yo'q</p>
                                        <p className="text-slate-700 text-xs mt-1">Do'kondan birinchi xaridingizni qiling</p>
                                    </div>
                                ) : (
                                    <div className="space-y-2">
                                        {orders.map(o => (
                                            ...
                                            >
                                                {/* Game image */}
                                                {o.image_url ? (
                                                    <img src={o.image_url} alt="" className="size-10 rounded-xl object-cover shrink-0 opacity-90" />
                                                ) : (
                                                ...
                                                <div className="flex-1 min-w-0">
                                                    <div className="flex items-center justify-between gap-2">
                                                        <p className="text-sm font-bold text-white truncate">{o.game_name}</p>
                                                        <p className="text-sm font-black text-white shrink-0">{fmt(o.price_uzs)}<span className="text-[10px] text-slate-600 font-normal ml-0.5">UZS</span></p>
                                                    </div>
                                                    <div className="flex items-center justify-between gap-2 mt-0.5">
                                                        <p className="text-xs text-slate-600 truncate">{o.variant_name}</p>
                                                        <StatusPill status={o.status} />
                                                    </div>
                                                    <div className="flex items-center gap-3 mt-1">
                                                        <span className="text-[10px] text-slate-700 font-mono">#{o.game_target}</span>
                                                        {o.zone_id && <span className="text-[10px] text-slate-700 font-mono">Z:{o.zone_id}</span>}
                                                        {o.saved_uzs > 0 && (
                    ...
                    {tab === "prices" && (
                        <div className="space-y-4 max-w-3xl">
                            <div className="flex items-center justify-between">
                                <h2 className="text-sm font-bold text-slate-400 uppercase tracking-widest">Reseller narxlari</h2>
                            </div>
                            ...
                            {Object.keys(products).length === 0 ? (
                                <div className="flex flex-col items-center justify-center py-16 text-center">
                                    ...
                                    <p className="text-slate-500 text-sm">Reseller narxlar belgilanmagan</p>
                                    <p className="text-slate-700 text-xs mt-1">Admin bilan bog'laning</p>
                                </div>
                            ) : Object.entries(products).map(([game, variants]) => (
                                <div key={game} className="bg-white/3 border border-white/5 rounded-2xl overflow-hidden">
                                    <div className="flex items-center gap-3 px-4 py-3 border-b border-white/5">
                                        {variants[0]?.image_url && (
                                            <img src={variants[0].image_url} alt={game} className="size-6 rounded-lg object-cover" />
                                        )}
                                        <p className="font-bold text-white text-sm">{game}</p>
                                        <span className="ml-auto text-[10px] text-slate-600">{variants.length} ta</span>
                                    </div>
                                    <div className="divide-y divide-white/3">
                                        {variants.map(p => (
                                            <div key={p.id} className="flex items-center justify-between px-4 py-3 hover:bg-white/3 transition-colors">
                                                <p className="text-sm text-slate-300 font-medium truncate flex-1 mr-4">{p.name}</p>
                                                <div className="text-right shrink-0 flex items-center gap-3">
                                                    <span className="text-xs text-slate-600 line-through">{fmt(p.price_uzs)}</span>
                                                    <div>
                                                        <p className="text-sm font-black text-white">{fmt(p.reseller_price_uzs)}<span className="text-[10px] text-slate-600 font-normal ml-0.5">UZS</span></p>
                                                        <p className="text-[10px] font-bold text-emerald-400 text-right">-{p.discount_percent}%</p>
                                                    </div>
                                                </div>
                                            </div>
                                        ))}
            ...
            <nav className="lg:hidden fixed bottom-0 left-0 right-0 z-40 bg-[#0f0f1a]/95 backdrop-blur-xl border-t border-white/6 px-2 py-2 flex items-center justify-around">
                ...
                <Link
                    href="/reseller/shop"
                    className="flex-1 flex flex-col items-center gap-1 py-1 rounded-xl text-slate-600 hover:text-slate-400 transition-all"
                >
                ...
                <Link
                    href="/reseller/balance"
                    className="flex-1 flex flex-col items-center gap-1 py-1 rounded-xl text-slate-600 transition-all"
                >

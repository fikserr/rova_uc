...
function Mlegends() {
    ...
    const handleAccountIdChange = (e) => {
        setMlAccountId(e.target.value);
        setVerifyStatus(null);
        setVerifyMessage("");
    };
    const handleServerIdChange = (e) => {
        setMlServerId(e.target.value);
        setVerifyStatus(null);
        setVerifyMessage("");
    };
    ...
    const handleVerify = async () => {
        if (!mlAccountId.trim() || !mlServerId.trim()) {
            ...
            setVerifyMessage(t("shop.ml_verify_enter_ids"));
            return;
        }
        setIsVerifying(true);
        setVerifyStatus(null);
    ...
    return (
        <div className="min-h-[calc(100vh-140px)] px-4 py-6 pb-8 lg:pb-8 dark:bg-slate-900">
            <Head title={t("shop.ml_page_title")} />
            {flash?.success && (
                        ...
                        <div className="relative w-full max-w-sm rounded-2xl border border-slate-200 bg-white p-6 dark:border-slate-700 dark:bg-slate-900 flex flex-col gap-5">
                            ...
                            <div className="rounded-xl border border-slate-100 bg-slate-50 px-4 py-3 flex flex-col gap-2.5 dark:border-slate-700 dark:bg-slate-800">
                                <div className="flex items-center justify-between">
                                    ...
                                    <span className="flex items-center gap-1.5 text-sm font-medium text-slate-900 dark:text-slate-100">
                                        <IoDiamondOutline className="size-4 text-blue-500" />
                                        {selectedProduct.diamonds} diamonds
                            ...
                            <div className="flex flex-col gap-3">
                                <div className="grid grid-cols-2 gap-3">
                                    <div>
                                        ...
                                        <input
                                            ...
                                            placeholder={t(
                                                "shop.account_id_placeholder",
                                            )}
                                            className={`h-10 w-full rounded-lg border px-3 text-sm text-slate-900 placeholder-slate-400 outline-none transition
                                                ${
                                ...
                                <button
                                    ...
                                    className="h-9 w-full rounded-lg border border-slate-200 bg-slate-50 text-xs font-medium text-slate-600 transition hover:bg-slate-100 hover:text-slate-900 disabled:opacity-50 disabled:cursor-not-allowed dark:border-slate-700 dark:bg-slate-800 dark:text-slate-400 dark:hover:bg-slate-700 dark:hover:text-slate-100 flex items-center justify-center gap-1.5"
                                >
                                    {isVerifying ? (
                                        <Loader2
                                            size={13}
                                    ...
                                    {isVerifying
                                        ? t("shop.verifying")
                                        : t("shop.ml_verify_btn")}
                                </button>

                                {verifyStatus === "success" && (
                                    <div className="flex items-center gap-2 rounded-lg border border-emerald-200 bg-emerald-50 px-3 py-2 dark:border-emerald-700 dark:bg-emerald-900/20">
                                        <CheckCircle
                                            size={14}

...
function UserBalance() {
    ...
    const STATUS_LABEL = {
        pending:  { text: t("balance.status.pending"),  cls: "bg-amber-50 text-amber-600 border border-amber-200 dark:bg-[#FFB020]/10 dark:text-[#FFB020] dark:border-[#FFB020]/20" },
        approved: { text: t("balance.status.approved"), cls: "bg-emerald-50 text-emerald-600 border border-emerald-200 dark:bg-[#10B981]/10 dark:text-[#10B981] dark:border-[#10B981]/20" },
        rejected: { text: t("balance.status.rejected"), cls: "bg-red-50 text-red-600 border border-red-200 dark:bg-[#EF4444]/10 dark:text-[#EF4444] dark:border-[#EF4444]/20" },
    };
    ...
    useEffect(() => {
        axios.get("/manual-topup/my").then((r) => setMyRequests(r.data)).catch(() => {});
    }, [checkSuccess]);

    useEffect(() => {
        axios.get("/payment-cards/active").then((r) => setPaymentCards(r.data)).catch(() => {});
    }, []);
                    ...
                    <div className="lg:col-span-8 space-y-5">
                        ...
                        <div className="rounded-2xl bg-white dark:bg-[#0F1021] p-5 border border-slate-200 dark:border-white/4 shadow-xl flex items-center justify-between">
                            <div>
                                ...
                                <span className="text-3xl font-bold text-slate-900 dark:text-white mt-1 block tracking-tight">
                                    ...
                                    <span className="text-sm font-semibold text-slate-400 dark:text-[#64748B]">uzs</span>
                                </span>
                            </div>
                        ...
                        <div className="space-y-3">
                            ...
                            <div className="grid grid-cols-2 gap-3">
                                ...
                                >
                                    <img src={ClickLogo} alt="Click" className="h-4 w-auto object-contain self-start" />
                                    <span className="font-bold text-xs text-slate-900 dark:text-white tracking-wide">Click</span>
                                </button>
                            ...
                            <div className="space-y-2.5">
                                ...
                                <div className="pt-1">
                                    <Swiper
                                        ...
                                        breakpoints={{ 480: { slidesPerView: 1 }, 768: { slidesPerView: 2 }, 1024: { slidesPerView: 2.2 } }}
                                        className="pb-8 h-52"
                                    >
                                                ...
                                                >
                                                    ...
                                                    <div className="relative h-full flex flex-col justify-between">
                                                        <div className="flex items-start justify-between">
                                                            ...
                                                            <span className="text-white font-extrabold text-sm tracking-wide drop-shadow-md">{card.bank_name ?? "UZCARD"}</span>
                                                        </div>
                                                        <div>
                                                            <p className="text-white/90 font-mono text-base tracking-[0.2em] font-bold">{card.card_number}</p>
                                                            <div className="flex items-center justify-between mt-2">
                                                                <p className="text-white/70 text-[10px] font-medium tracking-wide uppercase">{card.card_holder ?? "VALLER GAMING"}</p>
                                                                <button
                        ...
                        {selectedMethod === "click" && (
                            <div className="rounded-2xl p-5 bg-white dark:bg-[#0F1021] border border-slate-200 dark:border-white/4 shadow-xl space-y-5">
                                <div className="border-b border-slate-200 dark:border-white/4 pb-3 flex items-center justify-between">
                                    <h3 className="text-sm font-bold text-slate-900 dark:text-white uppercase tracking-wider">Click</h3>
                                    <Zap className="size-4 text-blue-500" />
                                </div>
                                <p className="text-xs text-slate-500 dark:text-slate-400">
                                    Click orqali tezda to'lang — to'lov tasdiqlangandan so'ng balans avtomatik tushadi.
                                </p>
                                <div className="space-y-1.5">
                                    ...
                                    <div className="relative">
                                        <input
                                            type="number"
                                            min="1000"
                                            placeholder={t("balance.amount_placeholder")}
                                            value={clickAmount}
                                        ...
                                        />
                                        <span className="absolute right-4 top-1/2 -translate-y-1/2 font-bold text-xs text-slate-400 font-mono">UZS</span>
                                    </div>
                                </div>
                                <div className="grid grid-cols-3 gap-2">
                                    {quickAmounts.slice(0, 6).map((amt) => (
                                        <button
                                            key={amt}
                                            onClick={() => setClickAmount(String(amt))}
                                ...
                                <button
                                    ...
                                    disabled={!clickAmount || parseFloat(clickAmount) < 1000 || clickLoading}
                            ...
                            <div className="rounded-2xl p-5 bg-white dark:bg-[#0F1021] border border-slate-200 dark:border-white/4 shadow-xl space-y-5">
                                ...
                                <div className="space-y-2">
                                    ...
                                    <ol className="space-y-1.5 text-xs text-slate-500 dark:text-[#64748B] font-medium">
                                        {["balance.manual_step1", "balance.manual_step2", "balance.manual_step3"].map((key, i) => (
                                ...
                                <div className="pt-1">
                                    {!preview ? (
                                        ...
                                        >
                                            ...
                                            <span className="text-xs font-bold text-slate-700 dark:text-[#E2E8F0]">{t("balance.upload_label")}</span>
                                            <span className="text-[10px] text-slate-400 dark:text-[#475569] font-medium max-w-sm text-center px-4 leading-relaxed">{t("balance.upload_hint")}</span>
                                        </button>
                                    ) : (
                                        <div className="relative rounded-xl border border-slate-200 dark:border-white/10 bg-slate-100 dark:bg-black/40 p-3 flex justify-center items-center group shadow-inner">
                                            <img src={preview} alt="Chek" className="max-h-40 rounded-lg object-contain shadow-lg" />
                                            <button
                                                onClick={removeFile}
                                ...
                                <div className="grid grid-cols-3 gap-2 pt-0.5">
                                    {quickAmounts.slice(0, 3).map((amt) => (
                                        <button
                                            key={amt}
                                            onClick={() => setCheckAmount(String(amt))}
                                            className={`py-2 px-1 text-xs font-mono font-bold rounded-xl border transition-all text-center ${
                                        ...
                                        >
                                            {Number(amt).toLocaleString("fr-FR")}{" "}
                                            <span className="text-[10px] font-sans font-medium text-slate-400">UZS</span>
                                        </button>
                                    ))}
                                </div>
                                ...
                                <button
                                    ...
                                    disabled={!receiptFile || !checkAmount || parseFloat(checkAmount) < 1000 || checkLoading}
                                ...
                                >
                                    {checkLoading ? (
                                        <><span className="size-3.5 border-2 border-white/20 border-t-white rounded-full animate-spin" /> {t("balance.submitting")}</>
                                    ) : (
                            ...
                            <table className="w-full text-left text-xs border-collapse">
                                <thead>
                                    <tr className="border-b border-slate-200 dark:border-white/4 text-slate-500 dark:text-[#475569] uppercase tracking-wider text-[10px] font-bold bg-slate-50 dark:bg-white/0.5">
                                        <th className="p-3 font-semibold">{t("balance.transferred_amount")}</th>
                                        <th className="p-3 font-semibold text-right">{t("balance.status_column")}</th>
                                    </tr>
                                </thead>
                                <tbody className="divide-y divide-slate-100 dark:divide-white/5">
                                    {myRequests.map((r) => {
                                        const s = STATUS_LABEL[r.status] ?? { text: r.status, cls: "bg-slate-100 text-slate-500 dark:bg-white/5 dark:text-slate-400" };
                                        return (
                                            <tr key={r.id} className="hover:bg-slate-50 dark:hover:bg-white/0.5 transition-colors">
                                                <td className="p-3">
                                                    <p className="font-mono font-bold text-slate-900 dark:text-white text-sm tracking-wide">
                                                        {Number(r.amount).toLocaleString("fr-FR")}{" "}
                                                        <span className="text-[11px] text-slate-400 dark:text-slate-500 font-sans">UZS</span>
                                                    </p>
                                                ...
                                                <td className="p-3 text-right">
                                                    <span className={`text-[9px] font-black uppercase tracking-wider px-2 py-0.5 rounded ${s.cls}`}>
                                                        {s.text}
                                                    </span>
                                                </td>
                                            </tr>
